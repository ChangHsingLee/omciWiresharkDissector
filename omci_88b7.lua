...
omciproto = Proto ("omci-onu", "OMCI ONU Protocol")
...
f.onu_protocol = ProtoField.bytes("omciproto.onu_protocol", "802 ONU extend OMCI protocol")
f.onu_len = ProtoField.bytes("omciproto.onu_len", "802 ONU extend OMCI protocol len")
...
function omciproto.dissector (buffer, pinfo, tree)
	...
	local offset = 0
	-- 802 ONU protocol
	local onu_protocol= buffer(offset, 5)
	subtree:add(f.onu_protocol, onu_protocol)
	offset = offset +  5
	local onu_len= buffer(offset, 2)
	subtree:add(f.onu_len, onu_len)
	offset = offset + 2
...
ether_table:add(0x88B7, omciproto) 
